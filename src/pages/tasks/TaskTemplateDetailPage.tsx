...
export function TaskTemplateDetailPage() {
  ...
  if (isPending) {
    return <div className="flex h-[400px] w-full items-center justify-center"><Spinner size="lg" color="white" /></div>
  }
  if (isError || !detail) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] gap-4">
        <p className="text-apple-text-tertiary">无法加载该任务模板详情</p>
        <Button variant="flat" onPress={() => navigate(-1)}>返回列表</Button>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-6 w-full text-apple-text-primary animate-in fade-in slide-in-from-bottom-4 duration-1000 max-w-[1280px] mx-auto pb-20 px-8 pt-8">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="flex items-center gap-3 mb-2 flex-wrap">
              ...
              {presetBadge ? <Chip size="sm" variant="flat" color={presetBadge.color as 'default' | 'warning'} classNames={{ base: 'text-[9px] font-black tracking-widest uppercase border-0' }}>{presetBadge.label}</Chip> : null}
              <Chip size="sm" variant="flat" color={detail.is_enabled ? 'success' : 'danger'} classNames={{ base: 'text-[10px] font-black tracking-widest uppercase border-0' }}>
                {detail.is_enabled ? '已启用' : '已禁用'}
              </Chip>
            </div>
            <p className="text-apple-text-secondary max-w-2xl">{detail.description || '暂无描述'}</p>
            {!detail.is_enabled && <p className="text-[12px] text-apple-amber mt-2">该模板当前已禁用，仅可查看，不能用于创建任务。</p>}
            {updateStatus.isError && <p className="text-[12px] text-apple-red-light mt-2">{updateStatus.error instanceof Error ? updateStatus.error.message : '模板状态更新失败'}</p>}
          </div>
          <div className="flex gap-3 flex-wrap">
            {canManageTemplates && (
              <Button
                variant="flat"
                color={detail.is_enabled ? 'danger' : 'success'}
                className="rounded-2xl h-12 px-6 font-black"
                isLoading={updateStatus.isPending}
                onPress={() => updateStatus.mutate({ code: detail.code, enabled: !detail.is_enabled })}
              >
                {detail.is_enabled ? '禁用模板' : '启用模板'}
              </Button>
            )}
            <Button
              color="primary"
      ...
      {isHighCost && <div className="mt-4 px-5 py-3 rounded-2xl bg-apple-amber/10 border border-apple-amber/20 text-[13px] text-apple-amber font-bold flex items-center gap-2"><span>⚠️</span> {FULL_PORT_WARNING}</div>}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-6">
              ...
              {detail.default_port_scan_mode === 'custom' && (
                <div className="col-span-2">
                  ...
                  <div className="text-[14px] font-mono whitespace-pre-wrap break-all">{detail.default_custom_ports || '未设置'}</div>
                </div>
              )}
              <div>
              ...
              {detail.supports_vul_scan && (
                <div className="col-span-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {detail.default_vul_scan_severity.map((severity) => <Chip key={severity} size="sm" variant="flat" className="bg-white/5 text-white border border-white/5">{getVulScanSeverityLabel(severity)}</Chip>)}
                  </div>
                </div>
              )}
              <div className="col-span-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {detail.default_stage_plan.map((stage) => <span key={stage} className="bg-white/5 px-3 py-1 rounded text-[12px] font-mono tracking-tight text-apple-text-secondary border border-white/5" title={stage}>{getRouteLabel(routes, stage)}</span>)}
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white/[0.02] border border-white/5 backdrop-blur-3xl rounded-[32px] p-8">
            <h2 className="text-[10px] uppercase font-black tracking-[0.2em] text-apple-text-tertiary mb-6">允许在发任务时覆盖的参数</h2>
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between"><span className="text-[13px] text-apple-text-secondary">端口模式覆盖</span>{detail.allow_port_mode_override ? <Chip size="sm" variant="dot" color="success" className="border-0 bg-transparent text-apple-text-primary font-bold">允许覆盖</Chip> : <Chip size="sm" variant="dot" color="default" className="border-0 bg-transparent text-apple-text-tertiary">模板写死</Chip>}</div>
              <div className="flex items-center justify-between"><span className="text-[13px] text-apple-text-secondary">首页识别开关</span>{detail.allow_http_probe_override ? <Chip size="sm" variant="dot" color="success" className="border-0 bg-transparent text-apple-text-primary font-bold">允许覆盖</Chip> : <Chip size="sm" variant="dot" color="default" className="border-0 bg-transparent text-apple-text-tertiary">模板写死</Chip>}</div>
              <div className="flex items-center justify-between"><span className="text-[13px] text-apple-text-secondary">高级参数 (并发/超时)</span>{detail.allow_advanced_override ? <Chip size="sm" variant="dot" color="success" className="border-0 bg-transparent text-apple-text-primary font-bold">允许覆盖</Chip> : <Chip size="sm" variant="dot" color="default" className="border-0 bg-transparent text-apple-text-tertiary">模板写死</Chip>}</div>
              {detail.supports_vul_scan && <div className="flex items-center justify-between"><span className="text-[13px] text-apple-text-secondary">漏洞等级</span>{detail.allow_advanced_override ? <Chip size="sm" variant="dot" color="success" className="border-0 bg-transparent text-apple-text-primary font-bold">允许覆盖</Chip> : <Chip size="sm" variant="dot" color="default" className="border-0 bg-transparent text-apple-text-tertiary">模板写死</Chip>}</div>}
            </div>
          </div>
        </div>
      </div>
