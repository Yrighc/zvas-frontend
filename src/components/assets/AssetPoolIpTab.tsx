    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 w-full mb-8">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between w-full gap-4">
        <div className="flex flex-col">
          <h3 className="text-xl font-black text-white tracking-tight mb-1">IP 视角网络指纹</h3>
          <p className="text-[13px] text-apple-text-tertiary font-medium">展示资产池中长期沉淀的 IP 级网络节点事实库，可用于暴露面监测溯源。</p>
        </div>
        ...
        <Table 
          ...
          <TableBody 
            emptyContent={
              <div className="py-20 text-apple-text-tertiary text-sm font-bold flex flex-col items-center gap-2">
                <span>此资产池下暂无 IP 事实资产 (NULL_IP_ASSETS)</span>
              </div>
            } 
            isLoading={isPending} 
          ...
          >
            {items.map((it) => (
              <TableRow key={it.id}>
                <TableCell>
                  <span className="font-mono text-[14px] text-apple-blue-light font-black tracking-tight drop-shadow-[0_0_8px_rgba(0,113,227,0.3)]">{it.display_name}</span>
                </TableCell>
                <TableCell>
