      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between w-full gap-4">
        <div className="flex flex-col">
          <h3 className="text-xl font-black text-white tracking-tight mb-1 flex items-center gap-2">
            ...
            <span>执行结转报告 (Reports)</span>
          </h3>
          <p className="text-[13px] text-apple-text-tertiary font-medium">查看基于本资产池范围所生成的可用格式报表归档。</p>
        </div>
      ...
      <div className="rounded-[32px] border border-white/10 bg-white/[0.02] backdrop-blur-3xl overflow-x-auto scrollbar-hide md:scrollbar-default custom-scrollbar">
        <Table 
          ...
          <TableHeader>
            ...
            <TableColumn width={160}>关联靶标类型</TableColumn>
            <TableColumn width={240}>关联靶标</TableColumn>
            <TableColumn width={120}>生产队列状态</TableColumn>
          ...
          <TableBody 
            emptyContent={
              <div className="py-20 text-apple-text-tertiary text-sm font-bold flex flex-col items-center gap-3">
                ...
                <span className="text-[13px] font-black tracking-[0.1em] text-white uppercase">NULL_REPORTS_ARCHIVE</span>
                <span className="text-[12px] text-apple-text-tertiary font-medium">当前资产池尚未出具备案报告记录。</span>
              </div>
            } 
            isLoading={isPending} 
          ...
          >
            {items.map((it) => (
              <TableRow key={it.id}>
                ...
                <TableCell>
                  <span className="font-mono text-[13px] text-white font-bold tracking-tight truncate block">{it.scope_name}</span>
                </TableCell>
                <TableCell>
