    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 w-full mb-8">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between w-full gap-4">
        <div className="flex flex-col">
          <h3 className="text-xl font-black text-white tracking-tight mb-1">域名树状指纹</h3>
          <p className="text-[13px] text-apple-text-tertiary font-medium">展示此资产池内完整演化的主域名与二级、三级关联域名图谱事实。</p>
        </div>
      ...
      <div className="rounded-[32px] border border-white/10 bg-white/[0.02] backdrop-blur-3xl overflow-x-auto scrollbar-hide md:scrollbar-default custom-scrollbar">
        <Table 
          ...
          <TableHeader>
            <TableColumn width={280}>完全限定域名 (FQDN)</TableColumn>
          ...
          <TableBody 
            emptyContent={
              <div className="py-20 text-apple-text-tertiary text-sm font-bold flex flex-col items-center gap-2">
                <span>此资产池下暂无域名事实资产 (NULL_DOMAIN_ASSETS)</span>
              </div>
            } 
            isLoading={isPending} 
        ...
        {total > 0 && (
          <div className="flex justify-between items-center px-6 py-5 border-t border-white/5 bg-white/[0.01]">
            <span className="text-[10px] uppercase font-black tracking-[0.2em] text-apple-text-tertiary">合计归集 <span className="text-white mx-1">{total}</span> 项域名实体</span>
            {totalPages > 1 && (
              <Pagination 
                size="sm" 
                page={page} 
