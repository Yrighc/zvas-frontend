...
function scopeLabel(scopeType: string, scopeID: string): string {
  switch (scopeType) {
    case 'task_vulnerability_word':
      return '漏洞扫描报告'
    case 'task_vulnerability_excel':
      return '漏洞扫描清单'
    default:
      return scopeID || '-'
  }
...
export function TaskReportsTab({ taskId }: { taskId: string }) {
  ...
  const { data, isPending, isError, refetch } = useTaskReports(taskId, {
    page,
    page_size: pageSize,
  })
  ...
  async function handleExport(format: 'word' | 'excel') {
    try {
      setExportError('')
      setExporting(format)
      await downloadTaskVulnerabilityReport(taskId, format)
      await refetch()
    } catch (error) {
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 w-full mb-8">
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between w-full gap-4">
        <div className="flex flex-col">
          ...
          <p className="text-[13px] text-apple-text-tertiary font-medium">导出当前任务的漏洞扫描 Word 报告和 Excel 清单，并查看导出历史。</p>
          ...
          <TableBody
            emptyContent={
              <div className="py-20 text-apple-text-tertiary text-sm font-bold flex flex-col items-center gap-3">
                ...
                <span className="text-[12px] text-apple-text-tertiary font-medium">当前任务还没有漏洞扫描报告导出记录。</span>
              </div>
            }
            isLoading={isPending}
          ...
          >
            {items.map((it) => (
              <TableRow key={it.id}>
                ...
                <TableCell>
                  <span className="font-mono text-[13px] text-white font-bold tracking-tight truncate block">{it.scope_id}</span>
                </TableCell>
                <TableCell>
